...
import com.daml.ledger.damlonxexample.Transaction._
...
class Ledger(timeModel: TimeModel, timeProvider: TimeProvider)(implicit mat: ActorMaterializer)
  ...
  ): Either[v1.Update, Unit] = {

    checkSubmission(submitterInfo, transactionMeta, txDelta, recordedAt, ledgerState)
      .flatMap(
        _ => runPreCommitValidation(submitterInfo, transactionMeta, transaction, ledgerState)
      )
  }
  ...
  private def checkDuplicates(
    ...
    Either.cond(
      !ledgerState.duplicationCheck
        .contains(submitterInfo.applicationId -> submitterInfo.commandId),
      (),
  ...
  private def mkContractOutputs(
      ...
      txDelta: TxDelta,
      offset: Offset
  ): Map[AbsoluteContractId, AbsoluteContractInst] = {
